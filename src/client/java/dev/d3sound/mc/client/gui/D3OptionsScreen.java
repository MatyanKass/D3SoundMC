...
public final class D3OptionsScreen extends OptionsSubScreen {
	 ...
	 *
	 * Тот набор, который стоит сейчас, отмечен галочкой. Если хоть один ползунок
	 * правился руками, галочки нет ни у кого: это уже свой набор, и пресеты его
	 * не трогают, пока не нажать. Нажимать можно любую кнопку, в том числе уже
	 * отмеченную — так набор возвращается на место после ручных правок.
	 */
	...
	private Button presetButton(D3Preset preset, D3Preset active) {
		...
		// раньше кнопка текущего набора гасилась, и это читалось как поломка:
		// «не даёт поставить». Какой набор стоит, видно по галочке, а нажать
		// можно любой — повторное нажатие просто вернёт набор на место, если
		// после него что-то правилось руками
		return button;
	}
